 ...
 */
// Uses global loadRules

self.sha1Hex = async function sha1Hex(str) {
    const enc = new TextEncoder();
    const buf = await crypto.subtle.digest('SHA-1', enc.encode(str));
    return Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join('');
}

self.buildCookieHashMap = async function buildCookieHashMap(cookies) {
    const map = {};
...
self.detectCookieSync = async function detectCookieSync(topSiteCookies, requestDetails, thirdPartyHost) {
